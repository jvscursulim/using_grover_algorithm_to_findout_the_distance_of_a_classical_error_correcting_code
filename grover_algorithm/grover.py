...
class GroverParityCheckSolver:
    ...
    def oracle(self) -> None:
        ...
        self.qc.h(self.qc.qregs[1][-1])
        self.qc.mct(control_qubits = self.qc.qregs[1][:-1], target_qubit = self.qc.qregs[1][-1])
        self.qc.h(self.qc.qregs[1][-1])
        self.qc.x(self.qc.qregs[1])
        self.qc.barrier()
    ...
    def get_classical_error_correction_code(self) -> dict:
        ...
        for key, value in statevector.probabilities_dict().items():
            ...
            if np.isclose(value, max_prob):
                
                code_words_list.append(key[self.parity_check_matrix.shape[0]:])
