...
class GroverParityCheckSolver:
    ...
    @staticmethod
    def find_codewords_and_code_distance(
        parity_check_matrix: np.ndarray,
        init_distance: int = 3,
        shots: int = 10000,
        shots_cutoff: int = 250,
        reps: int = 1,
        init_state: QuantumCircuit = None,
        stop_criteria: int = 20,
    ) -> tuple:
        ...
        Args:
            ...
            reps (int, optional): . Defaults to 1.
            init_state (QuantumCircuit, optional): . Defaults to None.
            stop_criteria (int, optional): . Defaults to 20.
        ...
        while not found_distance or counter >= stop_criteria:

            comparator = IntegerComparator(
                num_state_qubits=count_qubits, value=init_distance, geq=False
            )
            ...
            comparator_qubits = QuantumRegister(
                size=comparator.num_qubits, name="comparator"
            )
            ...
            qc = QuantumCircuit(
                qubits, ancilla, comparator_qubits, bits, bits_counts, bits_flag
            )

            if init_state is None:
                qc.h(qubit=qubits)
            else:
                qc = qc.compose(init_state, qubits)
            qc.barrier()
            ...
            for _ in range(reps):
                ...
                for i in range(parity_check_matrix.shape[0]):
                    for j in range(parity_check_matrix.shape[1]):
                        if parity_check_matrix[i][j] == 1:
                            qc.cx(control_qubit=qubits[j], target_qubit=ancilla[i])
                qc.barrier()

                qc.x(qubit=ancilla)
                qc.h(qubit=ancilla[-1])
                qc.mcx(control_qubits=ancilla[:-1], target_qubit=ancilla[-1])
                qc.h(qubit=ancilla[-1])
                qc.x(qubit=ancilla)
                ...
                if init_state is None:
                    ...
                    init_state_dg = init_state.inverse()
                    qc = qc.compose(init_state_dg, qubits)
                qc.h(qubit=qubits[-1])
            ...
            for i in range(parity_check_matrix.shape[1]):
                for j in range(count_qubits - 1, -1, -1):
                    if j == 0:
                        qc.cx(
                            control_qubit=qubits[i], target_qubit=comparator_qubits[j]
                        )
                    else:
            ...
            qc = qc.compose(
                other=comparator.decompose().decompose(), qubits=comparator_qubits
            )
